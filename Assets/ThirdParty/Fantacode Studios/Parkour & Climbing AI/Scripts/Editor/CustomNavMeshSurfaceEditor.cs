...
namespace FS_ParkourAI
{
    ...
    [CustomEditor(typeof(CustomNavMeshSurface))]
    class CustomNavMeshSurfaceEditor : UnityEditor.Editor
    {
        ...
        SerializedProperty m_BakeParkourActions;
        SerializedProperty m_ParkourHeight;
        ...
        void OnEnable()
        {
            ...
            m_ParkourHeight = serializedObject.FindProperty("parkourHeight");
            m_BakeParkourActions = serializedObject.FindProperty("bakeParkourLinks");
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.PropertyField(m_BakeParkourActions);

            if (m_BakeParkourActions.boolValue)
                EditorGUILayout.PropertyField(m_ParkourHeight);
            serializedObject.ApplyModifiedProperties();
            ...
            using (new EditorGUI.DisabledScope(Application.isPlaying || m_AgentTypeID.intValue == -1))
            {
                ...
                //var bakeOperations = NavMeshAssetManager.instance.IsSurfaceBaking();
                bool bakeInProgress = targets.Any(b =>
                {
                    return NavMeshAssetManager.instance.IsSurfaceBaking((NavMeshSurface)b);
                });
                ...
                else if (!bakeFinished)
                {
                    bakeFinished = true;
                    BakeParkourLink((target as CustomNavMeshSurface).navMeshData, target as CustomNavMeshSurface);
                }
            }
        }
        static bool bakeFinished = true;
        void BakeParkourLink(NavMeshData navMeshData,CustomNavMeshSurface target)
        {
            if (navMeshData == null) return;

            var serializedObject = new SerializedObject(navMeshData);

            serializedObject.Update();

            var offMeshLinks = serializedObject.FindProperty("m_OffMeshLinks");
            for (int i = 0; i < offMeshLinks.arraySize; i++)
            {
                var start = offMeshLinks.GetArrayElementAtIndex(i).FindPropertyRelative("m_Start");
                var end = offMeshLinks.GetArrayElementAtIndex(i).FindPropertyRelative("m_End");
                var direction = offMeshLinks.GetArrayElementAtIndex(i).FindPropertyRelative("m_LinkDirection");  
                var linkType = offMeshLinks.GetArrayElementAtIndex(i).FindPropertyRelative("m_LinkType");
                 
                //start.vector3Value = Vector3.zero;
                if (linkType.intValue == 1)
                {
                    var length = start.vector3Value.y - end.vector3Value.y;
                    if (length < m_ParkourHeight.floatValue)
                    {
                        
                        NavMesh.SamplePosition(target.transform.TransformPoint(end.vector3Value), out var hit, 1, NavMesh.AllAreas); 
                        if (hit.mask == (1 << NavMesh.GetAreaFromName("Walkable")) || length <= 2)
                        {
